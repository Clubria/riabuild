...
| `shared-build/` | the one cargo build directory every checkout and worktree compiles into. Untracked, created by a `SessionStart` hook — `riabuild-cli/AGENTS.md` is the authority |
| `.agents/skills/` | repo skills — read the relevant one before the work it covers. `.claude/skills` is a compatibility symlink |
...
`riabuild-cli/AGENTS.md` for the reasoning and the storage rules. Nothing here loosens the
