...
| `check()` makes no network request either | The same rule as `CommandRunner`, on the hole `CommandRunner` does not cover. `testing::ctx()` builds a real `ApiClient` against the production URL, so a `check()` that calls riabuild-web does not fail in a unit test — it **passes, against production**, and the suite is green for a reason that has nothing to do with the code. It then fails on any runner without a network, and reports whatever production happens to answer that day. Load what the check needs onto `Ctx` in `provision` and read it there as a plain field: `Ctx::load_secret_scope` is the worked example. |
