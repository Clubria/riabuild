...
E2E_LOGIN="$(gh api /user --jq .login 2>/dev/null || true)"
[ -n "$E2E_LOGIN" ] || die "E2E_GITHUB_TOKEN could not read /user. It is expired, revoked, or not a user token."
...
MEMBERSHIP="$(gh api "/user/memberships/orgs/$E2E_ORG" --jq .state 2>/dev/null || true)"
