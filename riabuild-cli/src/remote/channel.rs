...
//!
//! `crate::channel` builds the parts — the laptop's agent, the rules about
...
//! spelled `crate::channel::` throughout. The two are one word apart and only
...
/// see `crate::channel`'s module doc.
...
async fn try_start(plan: &Plan<'_>) -> Result<Started> {
    ...
    let agent = crate::channel::laptop_agent(plan.runner.clone(), &plan.paths.bin_dir())?;
