...
//! `read_line` and echoes, which is why [`crate::ui::secret`] exists rather
//! than a reuse of it.
//!
...
pub fn store(
    ...
    Ok(crate::keychain::for_password(
        runner,
        account,
...
///
/// `ask` is a closure rather than a direct call into [`crate::ui::secret`] so
...
mod tests {
    ...
    fn a_password_and_a_session_are_two_accounts_for_one_server() {
        ...
        assert_ne!(account(&remote()), crate::keychain::remote_account(&hash));
    ...
    async fn a_saved_password_is_reused_without_asking_again() {
        ...
        let store = crate::keychain::MemoryKeychain::with_token("hunter2");
    ...
    async fn a_password_asked_for_once_is_remembered() {
        let asked = std::cell::Cell::new(false);
        let store = crate::keychain::MemoryKeychain::default();
    ...
    async fn a_key_passphrase_is_answered_but_never_written_down() {
        ...
        let asked = std::cell::Cell::new(false);
        let store = crate::keychain::MemoryKeychain::with_token("the-servers-password");
