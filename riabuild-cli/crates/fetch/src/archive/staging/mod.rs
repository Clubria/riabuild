...
//!
//! **`std::fs` throughout, and that is not an exemption from "all IO is
//! async".** Every entry point here runs inside `archive::off_the_reactor`,
//! which is a `spawn_blocking` — the same blocking pool `tokio::fs` would have
//! handed each call to, minus a hop per file across a ~130 MB tree. What that
//! buys is only paid for while nothing here is reachable from the reactor
//! thread: a caller added below `archive`'s `async` wrappers, or a wrapper that
//! stops wrapping, puts a recursive `remove_dir_all` back on the runtime and
//! stalls every other future in the process for as long as the disk takes.
...
// `check_link_target` is reached by name rather than through the module, so
// `unpack` below reads the same as it did when the walk lived in this file.
use links::check_link_target;

/// The action on every failure here: something else on this machine owns the
/// path riabuild is trying to land a toolchain at, or the disk will not take it.
const CLEAR_THE_PATH: &str = "Check that there is free disk space and that nothing else owns that directory — another \
     developer on this machine may be installing the same version — then run `riabuild` again.";
...
fn extract_tarball(bytes: &[u8], target: &Path, strip_components: usize) -> Result<()> {
    ...
    remove_tree(&staging).map_err(|error| {
        Failure::new(
            format!("clearing {} before unpacking into it", staging.display()),
            CLEAR_THE_PATH,
        )
        .detail(format!("{error}"))
    })?;
...
pub(super) fn staging_beside(target: &Path, tag: &str) -> PathBuf {
...
fn swap_into_place(staging: &Path, target: &Path) -> Result<()> {
    ...
    if let Err(error) = std::fs::rename(target, &stale) {
        ...
        return Err(Failure::new(
            format!("moving {} aside to install a new copy", target.display()),
            CLEAR_THE_PATH,
        )
        .detail(format!("{error}"))
        .into());
...
///
/// `archive::extract_member` lands a single *file* through this too, which is
/// why the lost-race branch below asks whether what appeared has the shape of
/// what was staged rather than hardcoding "a directory".
pub(super) fn install_staged(
    staging: &Path,
    target: &Path,
    set_aside: Option<&Path>,
) -> Result<()> {
    // Read before the rename, because after it there is nothing left to ask.
    let staged_a_tree = std::fs::symlink_metadata(staging).is_ok_and(|meta| meta.is_dir());
    ...
    // already succeeded. Only the shape riabuild staged counts: a *file* where
    // a toolchain tree belongs is not a toolchain, and accepting one would
    // report an install that cannot run.
    if std::fs::symlink_metadata(target).is_ok_and(|meta| meta.is_dir() == staged_a_tree) {
        if let Some(stale) = set_aside {
            let _ = remove_tree(stale);
        }
        return Ok(());
    ...
    Err(
        Failure::new(format!("installing {}", target.display()), CLEAR_THE_PATH)
            .detail(format!("{error}"))
            .into(),
    )
}
...
fn unpack(bytes: &[u8], target: &Path, strip_components: usize) -> Result<()> {
    ...
    std::fs::create_dir_all(target).map_err(|error| unpacking_into(target, &error))?;

    for entry in archive.entries().map_err(|e| super::unreadable(&e))? {
        let mut entry = entry.map_err(|e| super::unreadable(&e))?;
        let path = entry
            .path()
            .map_err(|e| super::unreadable(&e))?
            .into_owned();
        let link = entry
            .link_name()
            .map_err(|e| super::unreadable(&e))?
        ...
        // `safe_join` guards where an entry is *written*; a link entry also
        // says where it points, and `Entry::unpack` writes that through with no
        // validation at all — `target_base` is `None`, so a symlink is created
        // from the raw header field and a hard link resolves against the
        // filesystem root. Two entries were therefore enough to write anywhere
        // the developer can: `x -> /home/ada/.ssh` followed by
        // `x/authorized_keys`, both of which `safe_join` waves through because
        // neither archived *path* leaves `target`.
        if kind.is_symlink() || kind.is_hard_link() {
            check_link_target(target, &relative, link.as_deref(), kind.is_symlink())?;
        }
        ...
        if let Some(parent) = destination.parent() {
            std::fs::create_dir_all(parent).map_err(|error| unpacking_into(target, &error))?;
        }
        entry
            .unpack(destination)
            .map_err(|error| unpacking_into(target, &error))?;
    }
    Ok(())
}

/// The disk would not take what the archive holds. A read-only mount, a full
/// filesystem, a directory this account cannot write to — all things the
/// developer can see, and none of them a bug in riabuild.
fn unpacking_into(target: &Path, error: &std::io::Error) -> anyhow::Error {
    Failure::new(
        format!("unpacking a download into {}", target.display()),
        CLEAR_THE_PATH,
    )
    .detail(format!("{error}"))
    .into()
...
mod tests {
    ...
    /// An archive whose entries are links rather than files.
    ///
    /// `tar::Builder` refuses to write a `..` in an entry *path*, which is the
    /// outer layer `safe_join` is the inner layer of — but it writes any link
    /// *target* it is given without complaint, which is the whole point of
    /// these fixtures.
    fn tarball_of_links(entries: &[(&str, &str, tar::EntryType)]) -> Vec<u8> {
        let mut builder = tar::Builder::new(flate2::write::GzEncoder::new(
            Vec::new(),
            flate2::Compression::fast(),
        ));
        for (path, link, kind) in entries {
            let mut header = tar::Header::new_gnu();
            header.set_size(0);
    ...
    #[test]
    fn a_symlink_that_points_out_of_the_target_is_refused() {
        // `safe_join` guards where an entry is *written*, and a link entry is
        // written perfectly legally at a path inside the target — it is where
        // it *points* that leaves. `x -> ../../../ada` followed by `x/id_ed25519`
        // is two entries neither of which `safe_join` objects to, and
        // `Entry::unpack` used to create the link with no validation at all,
        // so the second entry wrote straight through it. The comment on
        // `safe_join` says safety must not rest on the digest check; this is
        // the hole that left.
        let outside = tempfile::TempDir::new().unwrap();
        std::fs::write(outside.path().join("id_ed25519"), b"ada's real key").unwrap();

        for link in ["../../../../../../../../etc", "/etc", "../elsewhere"] {
            let bytes = tarball_of_links(&[(
                "node-v22.23.1-darwin-arm64/x",
                link,
            ...
            let error = extract_node_tarball(&bytes, &target)
                .expect_err(link)
                .to_string();
            assert!(error.contains("refused to unpack"), "{link}: {error}");

            // Nothing staged survives a refusal, and nothing landed.
        ...
        assert_eq!(
            std::fs::read(outside.path().join("id_ed25519")).unwrap(),
            b"ada's real key"
        );
    }

    #[test]
    fn a_hard_link_that_points_out_of_the_target_is_refused() {
        // The same hole, one entry type over, and worse: `Entry::unpack` is
        // called with no `target_base`, so `fs::hard_link` resolves the raw
        // header field against the *process* working directory. A hard link
        // target is archive-root-relative, so any `..` in it leaves the tree.
        let bytes = tarball_of_links(&[(
            "node-v22.23.1-darwin-arm64/x",
            "../../../../etc/shadow",
            tar::EntryType::Link,
        ...
        let error = extract_node_tarball(&bytes, &target)
            .expect_err("a hard link out of the tree")
            .to_string();
        assert!(error.contains("refused to unpack"), "{error}");
    }

    #[test]
    fn the_relative_symlinks_node_actually_ships_still_unpack() {
        // The guard has to be a check on where a link *resolves*, not a ban on
        // `..`: Node's own tarball is full of `bin/npm ->
        // ../lib/node_modules/npm/bin/npm-cli.js`, which climbs out of `bin/`
        // and is entirely legitimate. Refusing every `..` would refuse the
        // archive this function exists to unpack.
        let mut builder = tar::Builder::new(flate2::write::GzEncoder::new(
            Vec::new(),
            flate2::Compression::fast(),
        ));
        let payload = b"the real npm";
        ...
        builder
            .append_data(
                &mut file,
                "node-v22.23.1-darwin-arm64/lib/node_modules/npm/bin/npm-cli.js",
                &payload[..],
            )
            .unwrap();
        ...
        builder
            .append_link(
                &mut link,
                "node-v22.23.1-darwin-arm64/bin/npm",
        ...
        let home = tempfile::TempDir::new().unwrap();
        let target = home.path().join("22.23.1");
        extract_node_tarball(&bytes, &target).expect("node's own layout");

        assert!(
            std::fs::symlink_metadata(target.join("bin/npm"))
                .unwrap()
                .is_symlink()
        );
        assert_eq!(
            std::fs::read(target.join("bin/npm")).unwrap(),
            b"the real npm"
        );
    }
