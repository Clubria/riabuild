...
//!
//! `list` and `primary` are here beside the two pieces every command
//! shares: how an invocation is pointed at one account, and how a change
//! to the list is made inside the config lock. `create` and `remove` are
//! the other two, each large enough to be read on its own.
...
/// Applies one change to the account list **inside** the config lock.
///
/// Every command here used to mutate `ctx.config` — the snapshot this process
/// read at start — clone the resulting `Vec`, and have the closure assign it
/// wholesale. That is the lost update `UserConfig::update`'s own documentation
/// describes, reintroduced above it: `update` reads what is on disk under the
/// lock precisely so there is no stale snapshot to merge, and a closure that
/// ignores what it was handed throws that away. Two terminals running `claude
/// new`, `delete` or `primary` therefore each wrote a list missing the other's
/// edit, leaving an account directory on disk that the registry does not
/// number — and since **position is the number**, `claude_accounts` later
/// adopts that orphan and `claude-2` opens a different account than it did
/// yesterday.
///
/// With the mutation inside, the cap check, the position lookup and the number
/// handed back are all taken against the list that is actually there. The
/// closure returns a `Result` because `accounts::add` refuses at `MAX` and that
/// refusal has to be the one the disk justifies, not the one the snapshot did;
/// `update_config` cannot carry it, so it comes back out through `outcome`.
pub(crate) async fn update_accounts<T>(
    ctx: &mut Ctx,
    change: impl FnOnce(&mut riabuild_paths::config::UserConfig) -> Result<T>,
) -> Result<T> {
    let mut outcome = None;
    ctx.update_config(|config| outcome = Some(change(config)))
        .await?;
    // `expect` here would be a panic in a provisioner over an invariant this
    // function owns: `update_config` runs its closure or returns an error
    // through the `?` above, so `None` is unreachable. Unreachable is not the
    // same as unrepresentable, and the whole cost of saying so with `?` is this
    // sentence.
...
pub async fn primary(ctx: &mut Ctx, number: usize) -> Result<i32> {
    // Resolved for its refusal *and* for its value: `--check` has to report
    // "there is no account 4" rather than claim a promotion that would not
    // happen, and the id is what the promotion under the lock names.
    ...
    update_accounts(ctx, |config| {
        // Re-resolved from the id under the lock, so a list another terminal
        // reordered in the meantime promotes the account the developer named
        // rather than whatever has since moved into that position. Gone
        // entirely is a refusal, not a silent no-op: the `list` below would
        // otherwise print an unchanged box under a command that reported
        // success.
        if accounts::promote_id(config, &id) {
            return Ok(());
        }
        Err(Failure::new(
            format!("making Claude Code account {number} the one `claude` runs"),
            "Run `riabuild claude` to see the accounts you have.",
        )
        .detail("another riabuild removed that account while this one was working")
        .into())
    })
    .await?;
...
mod tests {
    ...
    /// What a *second* riabuild, in another terminal, did while this one was
    /// working: it registered an account of its own.
    ///
    /// Written straight through `UserConfig::update` because the writer's
    /// identity is not what is under test — `claude new`, `claude_accounts`
    /// adopting a directory it found, and a `riabuild remote` on the same
    /// machine all land here. What is under test is whether the command running
    /// in *this* process preserves it.
    async fn another_riabuild_adds_an_account(ctx: &Ctx) -> String {
        let id = accounts::new_id();
        tokio::fs::create_dir_all(ctx.paths.claude_profile_dir(&id))
            .await
            .unwrap();
        let registered = id.clone();
    ...
    /// A delete keeps an account another terminal registered in the meantime.
    ///
    /// The lost update this file used to have: `ctx.config` is the snapshot
    /// this process read at start, and assigning the whole `Vec` back inside
    /// `update_config` overwrites whatever landed since — so the fourth account
    /// disappeared from the registry while its directory stayed on disk.
    /// `claude_accounts` then adopts that orphan on the next run, and because
    /// **position is the number** it comes back at the end of the list:
    /// `claude-2` afterwards opens an account the developer never pointed it
    /// at.
    #[tokio::test]
    async fn a_delete_keeps_an_account_another_terminal_added() {
        let (mut ctx, _home, ids) = with_accounts(3).await;
        let added = another_riabuild_adds_an_account(&ctx).await;
        assert_eq!(
            ctx.config.claude_accounts, ids,
            "this process must still be holding its stale snapshot"
        );

        assert_eq!(delete(&mut ctx, 2, true).await.unwrap(), 0);

        assert_eq!(
            ctx.config.claude_accounts,
            vec![ids[0].clone(), ids[2].clone(), added]
        );
    }

    /// And so does an add, which additionally has to number itself against the
    /// list on disk rather than against the snapshot.
    #[tokio::test]
    async fn a_promotion_keeps_an_account_another_terminal_added() {
        let (mut ctx, _home, ids) = with_accounts(3).await;
        let added = another_riabuild_adds_an_account(&ctx).await;
        ...
        assert_eq!(
            ctx.config.claude_accounts,
            vec![ids[2].clone(), ids[0].clone(), ids[1].clone(), added]
        );
    }
    ...
    impl riabuild_runner::CommandRunner for NothingInstalled {
        async fn run(
            &self,
            program: &str,
            _args: &[&str],
