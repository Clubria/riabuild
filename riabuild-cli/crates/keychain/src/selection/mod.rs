//! Which store this machine gets, and in what order the question is asked.
//!
//! Three public entry points — [`for_platform`] for this machine's own session
//! token, [`for_password`] for a server's SSH password, [`for_account`] for the
//! laptop's cache of a server's session — over one [`Choice`] enum and one
//! [`store_for`] that builds what a `Choice` names.
//!
//! The enum and that one match are next door in `choice`, because building a
//! store asks no platform question at all. What stays here is everything that
//! does.
//!
//! Every one of the three takes the platform as a **parameter** rather than
//! asking `cfg!`, and so does the decision underneath it. That is not tidiness:
//! `cfg!` compiles every branch but the host's out of the test binary, so only
//! the runner's own answer could ever be asserted. The wrappers and
//! `each_wrapper_passes_the_platform_it_is_actually_running_on` therefore live
//! in one file, because a parameter without that test *moves* the untested
//! branch into the wrapper rather than removing it.

mod choice;

use crate::Keychain;
use crate::platform;
use choice::{Choice, store_for};
...
async fn for_account_on(
    ...
    // `select_password_store` returns neither `Env` nor `ServerFile`;
    // `store_for` handles them anyway so there is one exhaustive match.
    store_for(
        select_password_store(is_macos, answers, keyringless_fallback),
        runner,
        Some(account),
    )
}
...
async fn for_password_on(
    ...
    let answers = platform::keyring_answers(runner.as_ref()).await;
    // `select_password_store` returns neither `Env` nor `ServerFile` — the
    // session-token override and a server namespace both have no meaning for a
    // server's SSH password — but `store_for` covers them, so a new `Choice`
    // is a compile error in one place rather than a silent gap in three.
    store_for(
        select_password_store(is_macos, answers, fallback),
        runner,
        Some(account),
    )
}
