...
impl FileLock {
    ...
    pub async fn acquire(path: &Path, on_wait: impl FnOnce()) -> Result<Self> {
        ...
        // Opened through tokio and handed over, because the locking methods live
        // on `std::fs::File` and `tokio::fs::File` does not have them. This
        // handle exists to be locked: nothing is ever read or written through
        // it. `read` as well as `write` because Windows refuses to lock a handle
        // opened append-only, and it costs nothing on unix.
