...
pub use runtime_dir::choose_runtime_dir;
...
impl GhSession {
    ...
    pub async fn open(runtime: &Path, member_id: &str, pid: u32) -> Result<GhSession> {
        ...
        tokio::fs::write(&marker, riabuild_paths::config::now_secs().to_string()).await?;
    ...
    pub async fn close(self, runner: Arc<dyn CommandRunner>) -> Result<()> {
        ...
        sweep(&self.dir, runner, riabuild_paths::config::now_secs()).await?;
        Ok(())
    }
}
...
mod tests {
    ...
    async fn a_recycled_pid_cannot_keep_a_stale_tree_alive_forever() {
        ...
        // would look fresh.
        let a_week_later = riabuild_paths::config::now_secs() + 8 * 24 * 60 * 60;
