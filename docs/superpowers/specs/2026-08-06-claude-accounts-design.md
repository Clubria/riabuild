...
> **Superseded, 2026-08-28.** The launcher below is no longer a shell script. Every one of
> the decisions in it — the executable test, the `PATH` strip, the `--settings` guard —
> is Rust now, in `riabuild_tasks::shims::launch` and `shims::claude::launcher::handoff`,
> and the file in `~/.riabuild/bin` is one `exec` of `riabuild internal launch claude`
> carrying the same values. Nothing about *what* a launcher does changed; where it is
> decided did. This listing is kept because the reasoning under it — why the path is
> absolute, why the fallback strips `bin/`, why `paste -sd: -` rather than `tr '\n' ':'` —
> is still exactly why those functions are written the way they are. See
